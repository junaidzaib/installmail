...
/**
 * FieldBuilderTypeNumberResponder
 * 
...
class FieldBuilderTypeNumberResponder extends CBehavior
{
    ...
    protected function getValueModels()
    {
        ...
        $models = SurveyFieldNumber::model()->findAllByAttributes(array(
    ...
    protected function buildFieldArray($model)
    {
        ...
        if ($field->visibility == SurveyFieldNumber::VISIBILITY_VISIBLE || Yii::app()->apps->isAppName('customer')) {
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        $valueRule = array('value', 'numerical', 'integerOnly' => true, 'min' => $min, 'max' => $max);
        if ($event->sender->field->scenario === SurveyFieldNumber::SCENARIO_INTEGER_AND_FLOAT) {
            $valueRule = array('value', 'numerical', 'min' => $min, 'max' => $max);
        }
