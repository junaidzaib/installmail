...
/**
 * FieldBuilderTypeYearsRangeResponder
 * 
...
class FieldBuilderTypeYearsRangeResponder extends CBehavior
{
    ...
    {
        $field = $model->field;
        $this->prepareSurveyFieldModel($field);
        ...
        if ($field->visibility == SurveyField::VISIBILITY_VISIBLE || Yii::app()->apps->isAppName('customer')) {
        	$yearStart = $field->yearMin;
        	$yearEnd   = $field->yearMax;
	        $yearStep  = 1;
        	
	        if ($field->yearStart > 0) {
	        ...
	        if ($field->yearStep > 1) {
		        $yearStep = $field->yearStep;
	        }

            /* Make sure we don't go out of step range */
            $yearStep = $yearEnd - $yearStart < $yearStep ? 1 : $yearStep;
	        
        	$range = range($yearStart, $yearEnd, $yearStep);
	        $range = array_combine($range, $range);
            $fieldHtml = $this->owner->renderInternal($viewFile, compact('model', 'field', 'range'), true);
        }
    ...
    {
        // get the CSurvey instance of rules.
