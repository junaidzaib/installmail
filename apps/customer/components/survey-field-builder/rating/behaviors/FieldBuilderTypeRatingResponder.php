...
/**
 * FieldBuilderTypeRatingResponder
 * 
...
class FieldBuilderTypeRatingResponder extends CBehavior
{
    ...
    protected function getValueModels()
    {
        ...
        $models = SurveyFieldRating::model()->findAllByAttributes(array(
    ...
    protected function buildFieldArray($model)
    {
        ...
        if ($field->visibility == SurveyFieldRating::VISIBILITY_VISIBLE || Yii::app()->apps->isAppName('customer')) {
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        $rules->add(array('value', 'numerical', 'integerOnly' => true, 'min' => 0, 'max' => $max));
    }
