...
class FieldBuilderTypeTextResponder extends CBehavior
{
    ...
    protected function getValueModels()
    {
        ...
        $models = SurveyFieldText::model()->findAllByAttributes(array(
    ...
    protected function buildFieldArray($model)
    {
        ...
        if ($field->visibility == SurveyFieldText::VISIBILITY_VISIBLE || Yii::app()->apps->isAppName('customer')) {
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        $rules->add(array('value', 'length', 'min' => $min, 'max' => $max));
    }
